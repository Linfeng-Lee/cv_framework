...
class EmbeddingTrainer(abc.ABC):
    ...
    def init_trainer(self, model_names, lr, train_data_path, val_data_path,
                     train_transforms, val_transforms, gpus, classes,
                     batch_size, workers,criterion_list=None, tensorboard_save_path='temp/tensorboard', pretrained=False, **kwargs):
    ...
    def create_model(self, model_names, pretrained, **kwargs):
        model = network.__dict__[model_names](pretrained=pretrained, **kwargs)
        return model

    def resume(self, model_path="temp/weights/checkpoint.pth.tar", strict=False, **kwargs):
    ...
    def save_checkpoint(self, state, is_best, save_path="temp/weights"):
    ...
    def define_scalar(self, save_path="temp/tensorboard", comment="", **kwargs):
    ...
    def test_mask(self, show_channel, score_threshold, area_threshold, args,
                ...
                if True:
                    ...
                    for idx, mask_error_flag_item in enumerate(mask_error_flag):
                        ...
                        if mask_error_flag_item:
                            ...
                            cur_img_cpu=cv2.cvtColor(np.array(cur_img_cpu.transpose(1, 2, 0), dtype=np.uint8),cv2.COLOR_RGB2BGR)
                            dst = cv2.addWeighted(cur_img_cpu, 0.7,
                                                  mask_target_rgb, 0.3, 0)
                            cur_test_img_save_path = test_img_save_path.format(
                                "{}".format(img_name[:-4]) + ("_{}_a_area_{:.1f}".format(target[idx], mask_val_sum[idx])))
        ...
        for key, val in sum_nums_dict.items():
            ...
            mask_res_fs.writelines(
                "部位：{} 误割数量：{} 总部位数：{} 误割比例：{}\n".format(part_name[0], error_nums, cur_cls_sum_nums, scale))
        mask_res_fs.close()
        img_mask_info_fs.close()

    def test_img(self,show_channel,score_threshold,area_threshold,img_path_list,transforms,root_path,input_w,input_h,gpus,output_save_path="temp/output"):
        """
    ...
    def test_full_segmentation(self,transforms,root_path,input_w,input_h,gpus,classes,output_save_path="temp/output"):
        self.model_.eval()
        cost_time=[]
        with torch.no_grad():
