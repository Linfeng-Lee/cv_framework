...
class DownUpBone(nn.Module):
    def __init__(self,inplanes,out_channel):
        ...
        self.scale3 = nn.Sequential(
                                    nn.Conv2d(inplanes,inplanes, kernel_size=(3,3),groups=inplanes, stride=(2,2),padding=(1,1),bias=False),
                                    nn.BatchNorm2d(inplanes),
                                    nn.Conv2d(inplanes,out_planes,kernel_size=(1,1),bias=False),
                                    )
...
class MaskBranch(nn.Module):
    def __init__(self,concat_ch=12,mask_classes=1):
        ...
        self.conv21 = nn.Conv2d(232 + 116, concat_ch, kernel_size=(3, 3), padding=(1, 1), bias=False)
        self.bn21 = nn.BatchNorm2d(concat_ch)
        self.relu21 = nn.ReLU(inplace=True)

        self.conv12 = nn.Conv2d(24 + concat_ch * 2, concat_ch, kernel_size=(3, 3), padding=(1, 1), bias=False)
        ...
        self.conv22 = nn.Conv2d(116 + concat_ch * 2, concat_ch, kernel_size=(3, 3), padding=(1, 1), bias=False)
        self.bn22 = nn.BatchNorm2d(concat_ch)
        ...
        self.conv22_1 = nn.Conv2d(concat_ch, mask_classes, kernel_size=(1, 1))
...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x: Tensor):
        cls1, x1, x2, x3 = self._forward_impl(x)#, x4
        # mask1,mask2 = self.mask_branch(x1,x2,x3,x4)#
        x0_1_out = self.mask_branch(x1,x2,x3) #,x4
        x0_1_out = torch.argmax(x0_1_out, dim=1).unsqueeze(1).float()
        return [cls1, x0_1_out]
