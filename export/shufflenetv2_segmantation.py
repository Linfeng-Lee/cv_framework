...
class OneClassMaskBranch(nn.Module):
    def __init__(self, concat_ch=12):
        super(OneClassMaskBranch, self).__init__()
        ...
        # 网络的输入
        self.conv_d0 = nn.Conv2d(concat_ch * 2, 1, kernel_size=(3, 3), padding=(1, 1))
        self.downup = DownUpBone(concat_ch, concat_ch)
        # 辅助网络输出
        self.conv_d1 = nn.Conv2d(concat_ch, 1, kernel_size=(3, 3), padding=(1, 1))
        # 定义upsample
...
class MaskBranch(nn.Module):
    ...
    def forward(self,x1,x2,x3,x4):
        ...
        return F.softmax(x,dim=1).argmax(dim=1,keepdim=True),F.softmax(x_d1,dim=1).argmax(dim=1,keepdim=True)
...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        self.mask_branch = MaskBranch(mask_classes=num_classes)
